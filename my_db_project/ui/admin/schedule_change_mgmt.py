...
class ScheduleChangeMgmtWidget(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        subtitle = QLabel("管理员审批教师调课申请：批准后将同步更新授课时间")
        ...
        card_layout.addWidget(QLabel("审批意见"))
        self.edit_comment = QTextEdit()
        self.edit_comment.setPlaceholderText("可选：填写审批意见（拒绝时建议必填）")
        self.edit_comment.setFixedHeight(72)
        card_layout.addWidget(self.edit_comment)
    ...
    def refresh_requests(self) -> None:
        ...
        try:
            rows = AdminService.list_reschedule_requests(status if status else None)
        ...
        for r in rows:
            ...
            vals = [
                ...
                str(r.get("original_time") or "—"),
                str(r.get("requested_time") or "—"),
                status_map.get(str(r.get("status") or ""), str(r.get("status") or "")),
                str(r.get("admin_comment") or "—"),
        ...
        self.btn_approve.setEnabled(False)
        self.btn_reject.setEnabled(False)
        self.status_label.setText(f"已加载 {len(rows)} 条调课申请")
    ...
    def _approve(self) -> None:
        ...
        try:
            AdminService.process_reschedule_request(req_id, "APPROVED", comment)
            QMessageBox.information(self, "成功", "已批准调课申请并同步授课时间")
            self.refresh_requests()
    ...
    def _reject(self) -> None:
        ...
        comment = self.edit_comment.toPlainText().strip()
        if not comment:
            QMessageBox.information(self, "提示", "拒绝申请时请填写审批意见")
            return
        try:
            AdminService.process_reschedule_request(req_id, "REJECTED", comment)
            QMessageBox.information(self, "成功", "已拒绝调课申请")
            self.refresh_requests()
        except (ValidationError, BusinessError) as exc:
            QMessageBox.warning(self, "审批失败", str(exc))
